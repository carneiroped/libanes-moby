...
async function createAccountAndUser(userId: string) {
  // 2. Criar account
  ...
  // 5. Criar estágios do pipeline
  console.log('\n5️⃣ Criando estágios do pipeline...')
  const stages = [
    { name: 'Lead Novo', color: '#3b82f6', order: 1, is_closed_won: false, is_closed_lost: false },
    { name: 'Qualificação', color: '#8b5cf6', order: 2, is_closed_won: false, is_closed_lost: false },
    { name: 'Apresentação', color: '#06b6d4', order: 3, is_closed_won: false, is_closed_lost: false },
    { name: 'Proposta', color: '#10b981', order: 4, is_closed_won: false, is_closed_lost: false },
    { name: 'Negociação', color: '#f59e0b', order: 5, is_closed_won: false, is_closed_lost: false },
    { name: 'Fechado - Ganho', color: '#22c55e', order: 6, is_closed_won: true, is_closed_lost: false },
    { name: 'Fechado - Perdido', color: '#ef4444', order: 7, is_closed_won: false, is_closed_lost: true }
  ]
  ...
  console.log('\n6️⃣ Criando configurações iniciais...')
  const settings = [
  ...
  try {
    ...
    if (authError) {
      if (authError.message.includes('already registered')) {
        ...
        // Verificar se já tem account e user criados
        ...
        // Se não tem user criado, usa o existente
        const userId = existingUser.id
        console.log(`   ✅ Usando usuário existente: ${userId}`)

        // Continua criando account e user
        await createAccountAndUser(userId)
        return
      } else {
        throw authError
      }
